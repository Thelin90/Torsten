...
class TestTransactions(PySparkTest):
    ...
    def test_monthly_dataframe(self):
        """Test method verifies the process to create the monthly transaction dataframe with a fake spark session.
        ...
        _correct_monthly_df = TestDataFrames.CORRECT_MONTHLY_DF
        _incorrect_monthly_df = TestDataFrames.INCORRECT_MONTHLY_DF

        _df = RegularSpacedTransactions.transform_reguarly_spaced_transactions(_df, 'monthly', 27, 35)

        print('Assert against a correctly defined monthly schema named dataframe')
        self.assertEqual(_df.schema.names, _correct_monthly_df.schema.names)
        self.assertEqual(_df.dtypes, _correct_monthly_df.dtypes)

        print('Assert against a correctly defines monthly schema names in dataframe')
        self.assertEqual(_df.schema.names, _incorrect_monthly_df.schema.names)

        print('Assert that it had correct monthly schema names but wrong datatypes')
        self.assertNotEqual(_df.dtypes, _incorrect_monthly_df.dtypes)

        for col in _correct_monthly_df.schema.names:
            print('Assert that records in column: ' + col + ' is matching monthly dataframe')
            ...
            print('Assert that records in column: ' + col + ' is not matching monthly dataframe')
